    ...
    if vanilla_generation:
        ...
        for prompt, target_new in zip(prompts, targets):
            ...
            if locality:
                ...
                gen_tokens = gen_token.detach().cpu().numpy().tolist()[0]
                if len(gen_tokens) < len(target_new_tokens):
                    gen_tokens.extend([tok.eos_token_id]*(len(target_new_tokens)-len(gen_tokens)))
                results.append(np.mean(np.equal(target_new_tokens, gen_tokens[-len(target_new_tokens):])))
        return results
...
def PPL(
    ...
    with torch.no_grad():
        outputs = model(input_ids=input_ids.to(f"cuda:{device}"), labels=target_ids.to(f"cuda:{device}"))
        nll = outputs.loss
...
def answer_match(
    ...
    inputs = tok.encode(prompt, return_tensors='pt').to(f"cuda:{device}")
...
def per_generation(
    ...
    **kwargs
    ...
    def generate_text(query, model, tokenizer):
        ...
        input_ids = torch.tensor([src_input_ids], dtype=torch.long, device=f"cuda:{device}")
