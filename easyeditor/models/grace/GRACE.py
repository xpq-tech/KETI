...
class GRACE(torch.nn.Module):
    def __init__(self, config, model, device):
        ...
        if type(original_layer) is not GRACEAdapter:
            # self.grace_adapter = GRACEAdapter(config, original_layer, transpose=transpose).to(self.device)
            if config.adapter_path is not None:
                self.grace_adapter = torch.load(config.adapter_path)
            else:
                self.grace_adapter = GRACEAdapter(config, original_layer, transpose=transpose).to(self.device)
            setattr(edit_module, layer_name, self.grace_adapter)
            self.original_layer = copy.deepcopy(original_layer)

    def save_pretrained(self, path):
        edit_module = parent_module(self.model, brackets_to_periods(self.layer))
        layer_name = self.layer.rsplit(".", 1)[-1]
        grace_adapter = getattr(edit_module, layer_name)
        if type(grace_adapter) is not GRACEAdapter:
            raise ValueError(f"{grace_adapter} is not grace adapter")
        torch.save(grace_adapter, path+f"/{self.layer}.pt")
